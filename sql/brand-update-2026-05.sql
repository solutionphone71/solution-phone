...
BRAND LOGOS: The manufacturer logo (Apple bitten apple, Samsung wordmark, Xiaomi, Huawei, etc.) MUST appear on the back of the device shown — Solution Phone sells AUTHENTIC refurbished phones and a logo-less device looks like a counterfeit. However, NEVER show these brand logos elsewhere in the visual (not in titles, banners, footers) — Solution Phone is not Apple''s nor Samsung''s official partner.

FORBIDDEN: watermarks, rainbow gradients, RGB neon, 3D cartoon, cheap retail typography, cluttered backgrounds, generic stock photos, lens flare filters, over-saturation, emojis in image, fake Apple/Samsung headers or imitations of their corporate identity.
'::text))
